    ...
    for attempt in range(3):
        try:
            ...
            for binding in source_bindings:
                ...
                db.add(RunSourceDocument(run_id=run.id, source_document_id=source.id, role=binding.role, ordinal=binding.ordinal, included_text_hash=hashlib.sha256(included).hexdigest()))
...
def retry_run(db: Session, run_id: int) -> Run:
    ...
    bindings = [SourceBinding(source_document_id=item.source_document_id, role=item.role, ordinal=item.ordinal) for item in original.source_documents]
